...
const randArr = (arr) => { return arr[~~(Math.random() * arr.length)]; };


// Random RGB/HEX, max = threshold value (to prevent light shades)
const randHex = (max=16) => {
    return "#000000".replace(/0/g,function(){return (~~(Math.random()*max)).toString(16);});}
...
const randAngle = () => {return ~~(Math.random() * 360) + 'deg';}
...
const randRadialGradient = () => {
  ...
  /* possible = [
    ...
    ];*/ // - In practice, it turned out to be too redundant/abundand. In production version, it returns a bit simpler result.
  
    return 'radial-gradient(ellipse at ' + randPosition() + ', ' + randHex(14) + ', transparent)';
}
